Shader "Sample/CustomPixelShader"
{
    Properties
    {
        [Toggle(USE_MODEL_UV)] _Use_Metaball("Use Model UV", Int) = 0
        _Speed ("Speed", Float) = 1.0

        // Plateau inputs properties
	    _Side_MainTexture ("Side-MainTexture", 2D) = "white" {}
        _Side_MainColor ("SIde_MainColor", Color) = (255,255,255,255)
        ...
        _Top_MainColor ("Top_MainColor", Color) = (255,255,255,255)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #pragma shader_feature USE_MODEL_UV
            ...
            #include "PixelShaderFunction.hlsl"
            ...
            float4 frag(Varyings input) : SV_Target
            {
                ...
                // Caustics
                #ifdef USE_MODEL_UV
                const float2 causticsUV = input.uv;
                #else
                const float2 causticsUV = input.positionWS.xz * _Top_Titling;
                #endif

                float4 topColor = Caustics(causticsUV, _Speed);

                topColor.rgb = clamp(topColor.rgb + _Top_MainColor.rgb, 0., 1.);

                // StainedGlassLight
                #ifdef USE_MODEL_UV
                float2 stainedGlassUV = input.uv;
                #else
                float2 stainedGlassUV = input.positionWS.xy * _Side_Titling;
                #endif

                float4 sideColor=StainedGlassLight(stainedGlassUV, _Speed);
